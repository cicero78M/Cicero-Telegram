...
const {
  initializeTelegramDirektoratBot,
  stopTelegramDirektoratBot,
  getDirektoratBot,
  isDirektoratBotInitialized,
} = await import('../src/service/telegramDirektoratBotService.js');

describe('Telegram Direktorat Bot Service', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  afterEach(async () => {
    ...
    await stopTelegramDirektoratBot();
  });

  describe('initializeTelegramDirektoratBot', () => {
    it('should return null when bot is disabled', async () => {
      const bot = await initializeTelegramDirektoratBot('test-token', false);
      expect(bot).toBeNull();
      expect(MockTelegramBot).not.toHaveBeenCalled();
    });

    it('should return null when no token is provided', async () => {
      const bot = await initializeTelegramDirektoratBot('', true);
      expect(bot).toBeNull();
      expect(MockTelegramBot).not.toHaveBeenCalled();
    });

    it('should initialize bot with valid token and enabled flag', async () => {
      const bot = await initializeTelegramDirektoratBot('test-token', true);
    ...
    it('should setup command handlers during initialization', async () => {
      await initializeTelegramDirektoratBot('test-token', true);
  ...
  describe('stopTelegramDirektoratBot', () => {
    it('should stop the bot and reset state', async () => {
      await initializeTelegramDirektoratBot('test-token', true);
      expect(isDirektoratBotInitialized()).toBe(true);
      
      await stopTelegramDirektoratBot();
      
      expect(mockStopPolling).toHaveBeenCalled();
      expect(isDirektoratBotInitialized()).toBe(false);
      expect(getDirektoratBot()).toBeNull();
    });
  ...
  describe('getDirektoratBot', () => {
    ...
    it('should return bot instance when initialized', async () => {
      await initializeTelegramDirektoratBot('test-token', true);
      const bot = getDirektoratBot();
      expect(bot).not.toBeNull();
      expect(bot.sendMessage).toBe(mockSendMessage);
    });
  });

  describe('isDirektoratBotInitialized', () => {
